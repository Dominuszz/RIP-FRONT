...
import { getComplexClassById } from '../../store/slices/complexClassesSlice'
import { getDestImg } from "../../modules/target_config.ts"

export default function ComplexClassPage() {
    const dispatch = useAppDispatch()
    const { currentItem: compclass, loading, error } = useAppSelector(state => state.complexClasses)
    const [imageError, setImageError] = useState(false)
    const { id } = useParams()

    useEffect(() => {
        if (!id) return
        dispatch(getComplexClassById(Number(id)))
    }, [id, dispatch])
        ...
        return (
            <div className="compclass-page">
                ...
                <div className="compclass-page-not-found">
                    <h1>Ошибка загрузки класса сложности</h1>
                    <p>{error}</p>
                </div>
            </div>
        )
